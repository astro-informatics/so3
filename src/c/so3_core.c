...
void so3_core_mw_inverse_sov_sym(complex double *f, complex double *flm, 
  ...
  SO3_ERROR_MEM_ALLOC_CHECK(inds)
  ...
  for (el=abs(spin); el<=L-1; el++) {
    ...
    // Compute Fmm.
    elfactor = sqrt((double)(2.0*el+1.0)/(4.0*SO3_PI));
    el2pel = el *el + el;    
    for (m=-el; m<=el; m++)
  ...
  for (mm=-(L-1); mm<=L-1; mm++) {
    mmfactor = cexp(I*mm*SO3_PI/(2.0*L-1.0));
    for (m=-(L-1); m<=L-1; m++)
      Fmm[(mm + Fmm_offset)*Fmm_stride + m + Fmm_offset] *=
  	mmfactor;
  }
  ...
  */
  ...
  if (verbosity > 0) 
    printf("%s %s", SO3_PROMPT, "Inverse transform computed!");  
...
void so3_core_mw_inverse_sov_sym_real(double *f, complex double *flm, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_mw_inverse_sov_sym_real...");
  }
  ...
  SO3_ERROR_MEM_ALLOC_CHECK(Fmm_shift)
  for (mm=0; mm<=L-1; mm++)
    for (m=0; m<=L-1; m++)
  ...
  SO3_ERROR_MEM_ALLOC_CHECK(fext_real)
  fext_stride = 2*L-1;
...
void so3_core_mwdirect_inverse(complex double *f, complex double *flm, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_mwdirect_inverse...");
  }
  ...
  for (t=0; t<=L-1; t++) {
    theta = so3_sampling_mw_t2theta(t, L);   
    for (el=abs(spin); el<=L-1; el++) {	
      elfactor = sqrt((double)(2.0*el+1.0)/(4.0*SO3_PI));
      if (el!=0 && el==abs(spin)) {
	for(eltmp=0; eltmp<=abs(spin); eltmp++)
	...
	ssht_dl_beta_risbo_full_table(dl, theta, L, 
      ...
      for (m=-el; m<=el; m++) {
	so3_sampling_elm2ind(&ind, el, m);
	for (p=0; p<=2*L-2; p++) {
	  phi = so3_sampling_mw_p2phi(p, L);
	  f[t*f_stride + p] += 
	    ssign 
	    * elfactor 
	    * cexp(I*m*phi) 
...
void so3_core_mwdirect_inverse_sov(complex double *f, complex double *flm, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_mwdirect_inverse_sov...");
  }

  // Compute ftm.
  ftm = (complex double*)calloc(L*(2*L-1), sizeof(complex double));
  SO3_ERROR_MEM_ALLOC_CHECK(ftm)  
  ftm_stride = 2*L-1;
  ftm_offset = L-1;
  dlm1p1_line = (double*)calloc(2*L-1, sizeof(double));
  SO3_ERROR_MEM_ALLOC_CHECK(dlm1p1_line)
  dl_line = (double*)calloc(2*L-1, sizeof(double));
  SO3_ERROR_MEM_ALLOC_CHECK(dl_line)
  for (t=0; t<=L-1; t++) {
    theta = so3_sampling_mw_t2theta(t, L);   
    for (el=abs(spin); el<=L-1; el++) {	
      elfactor = sqrt((double)(2.0*el+1.0)/(4.0*SO3_PI));
...
void so3_core_mw_forward_sov_conv_sym(complex double *flm, complex double *f, 
  ...
  SO3_ERROR_MEM_ALLOC_CHECK(expsm)
  ...
  exps_offset = L-1;
  for (m=-(L-1); m<=L-1; m++)
    expsm[m + exps_offset] = cexp(I*SO3_PION2*(m+spin));
  for (mm=-(L-1); mm<=L-1; mm++)
    expsmm[mm + exps_offset] = cexp(-I*mm*SO3_PI/(2.0*L-1.0));
  ...
  for (mm=-2*(L-1); mm<=2*(L-1); mm++)
    w[mm+w_offset] = so3_sampling_weight_mw(mm);
  ...
  for (m=-(L-1); m<=L-1; m++) {
	...
	Fmm_pad[mm + w_offset] * 2.0 * SO3_PI / (4.0*L-3.0);
  ...
  // Print finished if verbosity set.
  if (verbosity > 0) 
    printf("%s %s", SO3_PROMPT, "Forward transform computed!");  
...
void so3_core_mw_forward_sov_conv_sym_real(complex double *flm, double *f, 
  ...
  exps_offset = L-1;
  for (m=0; m<=L-1; m++)
    expsm[m] = cexp(I*SO3_PION2*(m+spin));
  for (mm=-(L-1); mm<=L-1; mm++)
    expsmm[mm + exps_offset] = cexp(-I*mm*SO3_PI/(2.0*L-1.0));
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_mw_forward_sov_conv_sym_real...");
  }
  ...
  SO3_ERROR_MEM_ALLOC_CHECK(in_real)
  out = (complex double*)calloc(L, sizeof(complex double));
  SO3_ERROR_MEM_ALLOC_CHECK(out)
  ...
  for (el=abs(spin); el<=L-1; el++) {
    for (m=1; m<=el; m++) {
      so3_sampling_elm2ind(&ind, el, m);
      so3_sampling_elm2ind(&ind_nm, el, -m);
...
void so3_core_mw_inverse_sov_sym_pole(complex double *f, 
  ...
  SO3_ERROR_MEM_ALLOC_CHECK(f_full)

  // Perform inverse transform.
  so3_core_mw_inverse_sov_sym(f_full, flm, L, spin, 
			       dl_method, verbosity);	  
  ...
  *phi_sp = so3_sampling_mw_p2phi(0, L);
...
void so3_core_mw_inverse_sov_sym_real_pole(double *f, 
  ...
  // Perform inverse transform.
  so3_core_mw_inverse_sov_sym_real(f_full, flm, L, 
				    dl_method, verbosity);
...
void so3_core_mw_forward_sov_conv_sym_pole(complex double *flm, complex double *f,
  ...
  so3_core_mw_forward_sov_conv_sym(flm, f_full, L, spin, 
				    dl_method, verbosity);
...
void so3_core_mw_forward_sov_conv_sym_real_pole(complex double *flm, 
  ...
  // Perform forward transform.
  so3_core_mw_forward_sov_conv_sym_real(flm, f_full, L, 
					 dl_method, verbosity);
...
void so3_core_mw_inverse_sov_sym_ss(complex double *f, complex double *flm, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_mw_inverse_sov_sym_ss...");
  }
...
void so3_core_mw_inverse_sov_sym_ss_real(double *f, complex double *flm, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_mw_inverse_sov_sym_ss_real...");
  }
...
void so3_core_mwdirect_inverse_ss(complex double *f, complex double *flm, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_mwdirect_inverse_ss...");
  }
  ...
  dl_stride = ssht_dl_get_stride(L, SSHT_DL_FULL);    
  for (t=0; t<=L; t++) {
    theta = so3_sampling_mw_ss_t2theta(t, L);   
    for (el=abs(spin); el<=L-1; el++) {	
      elfactor = sqrt((double)(2.0*el+1.0)/(4.0*SO3_PI));
      if (el!=0 && el==abs(spin)) {
	for(eltmp=0; eltmp<=abs(spin); eltmp++)
	...
	so3_sampling_elm2ind(&ind, el, m);
	for (p=0; p<=2*L-1; p++) {
	  phi = so3_sampling_mw_ss_p2phi(p, L);
	  f[t*f_stride + p] += 
	    ssign 
	    * elfactor 
	    * cexp(I*m*phi) 
...
void so3_core_mw_forward_sov_conv_sym_ss(complex double *flm, complex double *f, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_mw_forward_sov_conv_sym_ss...");
  }
...
void so3_core_mw_forward_sov_conv_sym_ss_real(complex double *flm, double *f, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_mw_forward_sov_conv_sym_ss_real...");
  }
...
void so3_core_mw_inverse_sov_sym_ss_pole(complex double *f, 
  ...
  // Perform inverse transform.
  so3_core_mw_inverse_sov_sym_ss(f_full, flm, L, spin, 
				  dl_method, verbosity);	  
  ...
  *f_np = f_full[0];
  *phi_np = so3_sampling_mw_ss_p2phi(0, L);
  *f_sp = f_full[L*f_stride + 0];
  *phi_sp = so3_sampling_mw_ss_p2phi(0, L);
...
void so3_core_mw_inverse_sov_sym_ss_real_pole(double *f, 
  ...
  // Perform inverse transform.
  so3_core_mw_inverse_sov_sym_ss_real(f_full, flm, L, 
				       dl_method, verbosity);
...
void so3_core_mw_forward_sov_conv_sym_ss_pole(complex double *flm, complex double *f,
  ...
  // Perform forward transform.
  so3_core_mw_forward_sov_conv_sym_ss(flm, f_full, L, spin, 
				       dl_method, verbosity);
...
void so3_core_mw_forward_sov_conv_sym_ss_real_pole(complex double *flm, 
  ...
  // Perform forward transform.
  so3_core_mw_forward_sov_conv_sym_ss_real(flm, f_full, L, 
					    dl_method, verbosity);
...
void so3_core_gl_inverse_sov(complex double *f, complex double *flm, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_gl_inverse_sov...");
  }

  // Compute weights and theta positions.
  thetas = (double*)calloc(L, sizeof(double));
  SO3_ERROR_MEM_ALLOC_CHECK(thetas)
  weights = (double*)calloc(L, sizeof(double));
  SO3_ERROR_MEM_ALLOC_CHECK(weights)
  so3_sampling_gl_thetas_weights(thetas, weights, L);
...
void so3_core_gl_inverse_sov_real(double *f, complex double *flm, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_gl_inverse_sov_real...");
  }
...
void so3_core_gl_forward_sov(complex double *flm, complex double *f, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_gl_forward_sov...");
  }
  ...
  for (t=0; t<=L-1; t++) {
    ...
    for(m=0; m<=L-1; m++) 
      Ftm[t*Ftm_stride + m + Ftm_offset] = 
	inout[m] * 2.0 * SO3_PI / (2.0*L-1.0) ;
    for(m=-(L-1); m<=-1; m++) 
      Ftm[t*Ftm_stride + m + Ftm_offset] = 
	inout[m+2*L-1] * 2.0 * SO3_PI / (2.0*L-1.0);
  }
  fftw_destroy_plan(plan);
...
void so3_core_gl_forward_sov_real(complex double *flm, double *f, 
 ...
 // Print messages depending on verbosity level.
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_gl_forward_sov_real...");
  }
...
void so3_core_dh_inverse_sov(complex double *f, complex double *flm, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_dh_inverse_sov...");
  }
  ...
  SO3_ERROR_MEM_ALLOC_CHECK(dl_line)
  for (t=0; t<=2*L-1; t++) {
    theta = so3_sampling_dh_t2theta(t, L);
    for (el=abs(spin); el<=L-1; el++) {	
      elfactor = sqrt((double)(2.0*el+1.0)/(4.0*SO3_PI));
...
void so3_core_dh_inverse_sov_real(double *f, complex double *flm, 
  ...
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_dh_inverse_sov_real...");
  }
...
void so3_core_dh_forward_sov(complex double *flm, complex double *f, 
 ...
 // Print messages depending on verbosity level.
  if (verbosity > 0) {
    ...
    if (verbosity > 1)
      printf("%s %s\n", SO3_PROMPT, 
	     "Using routine so3_core_dh_forward_sov...");
  }
  ...
  for (t=0; t<=2*L-1; t++) {
    theta = so3_sampling_dh_t2theta(t, L);
    w = so3_sampling_weight_dh(theta, L);
    for (el=abs(spin); el<=L-1; el++) {
      elfactor = sqrt((double)(2.0*el+1.0)/(4.0*SO3_PI));
      el2pel = el *el + el;    
      for (m=-el; m<=el; m++)
