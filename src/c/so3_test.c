...
/*! 
 * \file so3_test.c
 * Applies SO3 algorithms to perform inverse and forward spherical
 ...
 * sampled from (-1,1).
 *
 * Usage: so3_test B spin, e.g. so3_test 64 2
 *
...
void so3_test_gen_flm_complex(complex double *flm, int L, int spin, int seed);
void so3_test_gen_flm_real(complex double *flm, int L, int seed);
...
int main(int argc, char *argv[]) {
  ...
  for (irepeat = 0; irepeat < NREPEAT; irepeat++) {
    ...
    if (spin == 0) {
      ...
      so3_test_gen_flm_real(flm_orig, L, seed);
      time_start = clock();
      so3_core_dh_inverse_sov_real(f_dh_real, flm_orig, L, verbosity);
      time_end = clock();
      durations_inverse_dh_real[irepeat] =
      	(time_end - time_start) / (double)CLOCKS_PER_SEC;
      
      time_start = clock();
      so3_core_dh_forward_sov_real(flm_syn, f_dh_real, L, verbosity);
      time_end = clock();
      ...
      so3_test_gen_flm_real(flm_orig, L, seed);
      time_start = clock();
      so3_core_gl_inverse_sov_real(f_gl_real, flm_orig, L, verbosity);
      time_end = clock();
      durations_inverse_gl_real[irepeat] =
      	(time_end - time_start) / (double)CLOCKS_PER_SEC;

      time_start = clock();
      so3_core_gl_forward_sov_real(flm_syn, f_gl_real, L, verbosity);
      time_end = clock();
      ...
      so3_test_gen_flm_real(flm_orig, L, seed);
      time_start = clock();
      so3_core_mw_inverse_sov_sym_real(f_mw_real, flm_orig, L, 
					dl_method, verbosity);      
      time_end = clock();
	...
	(time_end - time_start) / (double)CLOCKS_PER_SEC;
      ...
      time_start = clock();
      so3_core_mw_forward_sov_conv_sym_real(flm_syn, f_mw_real, L, 
					     dl_method, verbosity);      
      time_end = clock();
	...
	errors_mw_real[irepeat] = 
      ...
      so3_test_gen_flm_real(flm_orig, L, seed);
      time_start = clock();
      so3_core_mw_inverse_sov_sym_real_pole(f_mw_real_pole,
      					     &f_mw_real_sp,
      					     flm_orig, L,
      					     dl_method, verbosity);
      time_end = clock();
      ...
      time_start = clock();
      so3_core_mw_forward_sov_conv_sym_real_pole(flm_syn, f_mw_real_pole,
      						  f_mw_real_sp,
      						  L,
      						  dl_method, verbosity);
      time_end = clock();
      ...
      so3_test_gen_flm_real(flm_orig, L, seed);
      time_start = clock();
      so3_core_mw_inverse_sov_sym_ss_real(f_mw_ss_real, flm_orig, L,
      					   dl_method, verbosity);
      time_end = clock();
      ...
      time_start = clock();
      so3_core_mw_forward_sov_conv_sym_ss_real(flm_syn, f_mw_ss_real, L,
      						dl_method, verbosity);
      time_end = clock();
      ...
      so3_test_gen_flm_real(flm_orig, L, seed);
      time_start = clock();
      so3_core_mw_inverse_sov_sym_ss_real_pole(f_mw_ss_real_pole,
      						&f_mw_ss_real_np,
      						&f_mw_ss_real_sp,
      						flm_orig, L,
      						dl_method, verbosity);
      ...
      time_start = clock();
      so3_core_mw_forward_sov_conv_sym_ss_real_pole(flm_syn,
      						     f_mw_ss_real_pole,
      						     f_mw_ss_real_np,
      						     f_mw_ss_real_sp,
      						     L,
    ...
    so3_test_gen_flm_complex(flm_orig, L, spin, seed);
    time_start = clock();
    so3_core_dh_inverse_sov(f_dh, flm_orig, L, spin, verbosity);
    time_end = clock();
    durations_inverse_dh[irepeat] = (time_end - time_start) / (double)CLOCKS_PER_SEC;

    time_start = clock();
    so3_core_dh_forward_sov(flm_syn, f_dh, L, spin, verbosity);
    time_end = clock();
    ...
    so3_test_gen_flm_complex(flm_orig, L, spin, seed);
    time_start = clock();
    so3_core_gl_inverse_sov(f_gl, flm_orig, L, spin, verbosity);
    time_end = clock();
    durations_inverse_gl[irepeat] = (time_end - time_start) / (double)CLOCKS_PER_SEC;

    time_start = clock();
    so3_core_gl_forward_sov(flm_syn, f_gl, L, spin, verbosity);
    time_end = clock();
    ...
    so3_test_gen_flm_complex(flm_orig, L, spin, seed);
    time_start = clock();
    so3_core_mw_inverse_sov_sym(f_mw, flm_orig, L, spin, dl_method, verbosity);    
    time_end = clock();
    durations_inverse_mw[irepeat] = (time_end - time_start) / (double)CLOCKS_PER_SEC;

    time_start = clock();
    so3_core_mw_forward_sov_conv_sym(flm_syn, f_mw, L, spin, dl_method, verbosity);    
    time_end = clock();
    ...
    so3_test_gen_flm_complex(flm_orig, L, spin, seed);
    time_start = clock();
    so3_core_mw_inverse_sov_sym_pole(f_mw_pole, &f_mw_sp, &phi_sp,
    				      flm_orig, L, spin,
    				      dl_method, verbosity);
    time_end = clock();
    ...
    time_start = clock();
    so3_core_mw_forward_sov_conv_sym_pole(flm_syn, f_mw_pole, f_mw_sp, phi_sp,
    					   L, spin,
    					   dl_method, verbosity);
    time_end = clock();
    ...
    so3_test_gen_flm_complex(flm_orig, L, spin, seed);
    time_start = clock();
    so3_core_mw_inverse_sov_sym_ss(f_mw_ss, flm_orig, L, spin,
    				    dl_method, verbosity);
    //so3_core_mwdirect_inverse_ss(f_mw_ss, flm_orig, L, spin, verbosity);
    time_end = clock();
    durations_inverse_mw_ss[irepeat] = (time_end - time_start) / (double)CLOCKS_PER_SEC;

    time_start = clock();
    so3_core_mw_forward_sov_conv_sym_ss(flm_syn, f_mw_ss, L, spin,
    					 dl_method, verbosity);
    time_end = clock();
    ...
    so3_test_gen_flm_complex(flm_orig, L, spin, seed);
    time_start = clock();
    so3_core_mw_inverse_sov_sym_ss_pole(f_mw_ss_pole,
    					 &f_mw_ss_np, &phi_np,
    					 &f_mw_ss_sp, &phi_sp,
    					 flm_orig, L, spin,
    					 dl_method, verbosity);
    ...
    time_start = clock();
    so3_core_mw_forward_sov_conv_sym_ss_pole(flm_syn, f_mw_ss_pole,
    					      f_mw_ss_np, phi_np,
    					      f_mw_ss_sp, phi_sp,
    					      L, spin,
    					      dl_method, verbosity);
...
void so3_test_gen_flm_complex(complex double *flm, int L, int spin, int seed) {
  ...
  so3_sampling_elm2ind(&i_lo, abs(spin), 0);
  for (i=i_lo; i<L*L; i++) 
    flm[i] = (2.0*ran2_dp(seed) - 1.0) + I * (2.0*ran2_dp(seed) - 1.0);
