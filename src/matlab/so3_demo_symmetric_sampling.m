% so3_demo_symmetric_sampling - Run symmetric sampling demo.
%
% Simple demo to compute inverse and forward transform of complex scalar
% function, using McEwen and Wiaux symmetric sampling.
%
...
f = so3_inverse(flmn, L, N, 'Sampling', 'MWSS');
flmn_syn = so3_forward(f, L, N, 'Sampling', 'MWSS');

% Compute maximum error in harmonic space.
maxerr = max(abs(flmn_syn - flmn))

% Compute sampling grids.
[alphas, betas, gammas, n, nalpha, nbeta, ngamma] = so3_sampling(L, N, 'Grid', true, 'Method', 'MWSS');
...
for i = 1:ngamma,
    f_g = squeeze(f(i,:,:));
    subplot(1,ngamma,i)
    ssht_plot_sphere(abs(f_g), L, 'Method', 'MWSS');
