function [ f ] = so3_direct_inverse_1d( L, M, N, flmn )
...
for l = 0:L-1,
    ...
    for m = -l:l,
        for n = -l:l,
            sign = 1i^(n-m);
            for mp = -l:l, % m'
                Fmnm(L+mp + (2*L-1)*(N+n-1 + (2*N-1)*(M+m-1))) = Fmnm(L+mp + (2*L-1)*(N+n-1 + (2*N-1)*(M+m-1))) +...
                    sign*(2*l+1)/(8*pi^2)*...
